...
function countFiles(collectionName , response ){
    Grid(db , mongo).collection(collectionName).estimatedDocumentCount({}).then(function(num) {
 ...
 */ // working 
 function getApplicantByspNumber(applicantName, response) {
    db.collection("applicants").findOne({ "loginCred.spNumber" : applicantName.trim() } ,function(err,result){
        // console.log(result)
        if (err) response.send(err);
        else response.send(result);
    });
