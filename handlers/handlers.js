                    ...
                    db.collection("users").insertOne(document, function (err, result) {
                        ...
                        if (err) {
                            ...
                            if (typeof callback === "function") {
                                ...
                                generateAccountActivationToken(document, response);
...
function dropCV(response) {
    db.collection('CV').drop().then(function (val) {
...
function verifyUserLoggingCredentials(spNumber, password, response) {
    ...
    // db.collection("curriculum-vitae").deleteMany({}, function (err, result) {
                ...
                bcrypt.compare(password, result.password, function (err, same) {
                    ...
                    if (err) {
                        ...
                        if (same == true && result.activated == true) {
                            ...
                            db.collection("CV").findOne({ spNumber: spNumber.trim() }, function (err, result) {
...
function insertCv(spNumber, document, res) {
    ...
    // db.collection("curriculum-vitae").deleteMany({ spNumber: spNumber }, function (err, result){

    // });

    console.log(JSON.stringify(spNumber))
    ...
    try {
        ...
        db.collection("CV").findOne({ spNumber: spNumber.trim() }, function (err, result) {
            ...
            if (err) {
                ...
                res.send({ errorMsg: "Error Occurred while inserting the CV" })
                ...
                db.collection("CV").insertOne({ spNumber: spNumber.trim(), cv: JSON.stringify(document) }, function (err, result) {
                    ...
                    if (err) res.send(err);
                        ...
                        res.status(200).send({ "response": "Your curriculum vitae insert was successful" });
 ...
 * @param {*} response 
 */ //working
function checkIfAdminusernameExist(spNumber, response) {
            ...
            .insertOne(
                ...
                function (error, result) {
                    ...
                    if (error)
                        ...
                        console.log("CALLING YOU FROM  saveGeneratedToken" , dates[1])

                       

                        sendResetPasswordLink(document, dates[1], token, response);
 ...
 * 
 */
async function generateAccountActivationToken(document, response) {

    try {
        const tokenExpiringDate = new Date(Date.now() + (1000 * 24 * 60 * 60));
        const token = objectHash(document.email + Date.now());
        sendActivationLink(document, tokenExpiringDate , token, response);
...
function sendAnotherActivationLink(email, spNumber, response) {
    try {
        ...
        sendActivationLink(document , response);
    } catch (err) {
        console.log(err);
 ...
 * @param {*} response 
 */
function sendResetPasswordLink(document, tokenExpiringDate, token, response) {

    console.log('EMAIL ADRESS ::::: '  , document.email)
    mailer({
        email: [document.email, 'keemsisi@gmail.com'],
        message: "A mail has been sent to your email address to reset your account password"
            + "Sent at "
            + tokenExpiringDate.toDateString() + ",and then the token expires "
            + " at " + tokenExpiringDate.toTimeString() +
            ` . click on the link to reset password >>>>>  https://promotbotweb.herokuapp.com/#/resetpassword?email=${document.email}&token=${token}`,
        subject : "Account Forgot Password Reset Link"
    }).then(function (success) {
        response.status(200).send(
            {
                tokenGenrated: true,
                message: `A mail has been sent to your email address to change your account password kindly check to proceed!`
            }
        )
 ...
 * @param {*} response 
 */
function sendActivationLink(document, tokenExpiringDate, token, response) {
...
function gradeCV(spNumber, response) {
    ...
    try {
        ...
        db.collection("CV").findOne({ spNumber: spNumber }, function (err, result) {
